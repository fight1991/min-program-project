...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.setNavigationBarTitle({
      title: app.translater('计量单位查询'),
    })
    app.authorize()
    this.data.searchModel = app.searchModel
    if (typeof (options.unit_code) != "undefined") {
      this.data.searchModel["unit_code"] = options.unit_code
      this.data.searchModel["unit_name"] = options.unit_name
    }
    //显示上一次查询数据
  ...
  reset: function () {
    this.data.searchModel.unit_code = ""
    this.data.searchModel.unit_name = ""
    var searchModel = this.data.searchModel
    this.setData({
      searchModel: searchModel
    })
