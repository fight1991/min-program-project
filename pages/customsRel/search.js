...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.setNavigationBarTitle({
      title: app.translater('关区代码查询'),
    })
    app.authorize()
    this.data.searchModel = app.searchModel
    if (typeof (options.customs_code) != "undefined") {
      this.data.searchModel["customs_code"] = options.customs_code
      this.data.searchModel["customs_name"] = options.customs_name
    }
    //显示上一次查询数据
  ...
  reset: function () {
    this.data.searchModel.customs_code = ""
    this.data.searchModel.customs_name = ""
    var searchModel = this.data.searchModel
    this.setData({
      searchModel: searchModel
    })
