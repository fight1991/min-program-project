...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.setNavigationBarTitle({
      title: app.translater('国家(地区)查询'),
    })
    app.authorize()
    this.data.searchModel = app.searchModel
    if (typeof (options.country_co) != "undefined") {
      this.data.searchModel["country_co"] = options.country_co
      this.data.searchModel["country_na"] = options.country_na
    }
    //显示上一次查询数据
  ...
  reset: function () {
    this.data.searchModel.country_co = ""
    this.data.searchModel.country_na = ""
    var searchModel = this.data.searchModel
    this.setData({
      searchModel: searchModel
    })
