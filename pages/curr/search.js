...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.setNavigationBarTitle({
      title: app.translater('币制查询'),
    })
    app.authorize()
    this.data.searchModel = app.searchModel
    if (typeof (options.curr_code) != "undefined") {
      this.data.searchModel["curr_code"] = options.curr_code
      this.data.searchModel["curr_symb"] = options.curr_symb
    }
    //显示上一次查询数据
  ...
  reset: function () {
    this.data.searchModel.curr_code = ""
    this.data.searchModel.curr_symb = ""
    var searchModel = this.data.searchModel
    this.setData({
      searchModel: searchModel
    })
